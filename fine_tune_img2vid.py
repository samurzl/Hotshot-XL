...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default="0",
        help="Alpha mixing coefficient for mixing in condition frame.",
    )
    parser.add_argument(
        "--unet_resume_path",
        type=str,
    ...
    parser.add_argument("--unfreeze_spatial_layers", action="store_true")
...
def main():
    ...
    if not args.unfreeze_spatial_layers:
    ...
    if (args.unfreeze_spatial_layers):
        params_to_optimize = [
    ...
    def compute_loss_from_batch(batch: dict):
        ...
        with torch.no_grad():
            ...
            #noise[:, :, 0, :, :] = 0.0  # Assuming frame index 0 is the first frame
            ...
            reference_latent_1 = latents[:, :, 0, :, :]
            reference_latent_2 = latents[:, :, 1, :, :]
            reference_latent_3 = latents[:, :, 2, :, :]
            reference_latent_4 = latents[:, :, 3, :, :]
            ...
            # Blend with reference latent
            noisy_latents[:, :, 0, :, :] = args.alpha * noisy_latents[:, :, 0, :, :] + (1 - args.alpha) * reference_latent_1
            noisy_latents[:, :, 1, :, :] = args.alpha * noisy_latents[:, :, 1, :, :] + (1 - args.alpha) * reference_latent_2
            noisy_latents[:, :, 2, :, :] = args.alpha * noisy_latents[:, :, 2, :, :] + (1 - args.alpha) * reference_latent_3
            noisy_latents[:, :, 3, :, :] = args.alpha * noisy_latents[:, :, 3, :, :] + (1 - args.alpha) * reference_latent_4

            if noise_scheduler.config.prediction_type == "epsilon":
                target = noise
                # Set target for the first frame to zero since no noise was added
                #target[:, :, 0, :, :] = 0.0
            elif noise_scheduler.config.prediction_type == "v_prediction":
                target = noise_scheduler.get_velocity(latents, noise, timesteps)
                # Adjust target for the first frame accordingly
                #target[:, :, 0, :, :] = 0.0
            else:
